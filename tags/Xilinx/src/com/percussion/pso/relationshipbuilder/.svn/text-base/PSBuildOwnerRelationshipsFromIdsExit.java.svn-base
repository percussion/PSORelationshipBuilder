 ...
 * @since 6.0
 */
public class PSBuildOwnerRelationshipsFromIdsExit extends PSDefaultExtension // NOPMD by agent on 11/28/06 9:25 AM
      implements
         IPSResultDocumentProcessor
{
   ...
   public Document processResultDocument(final Object[] params,
			final IPSRequestContext request, final Document resultDoc)
			throws PSParameterMismatchException, PSExtensionProcessingException {
		PSAbstractRelationshipBuilder builder;
		try {
			builder = new PSOwnerRelationshipBuilder(
					new PSRelationshipProcessor(request));
			final PSRelationshipBuilderExitHelper helper = new PSRelationshipBuilderExitHelper(
					builder, this.getParameters(params));
			helper.processResultDocument(request, resultDoc);
		} catch (PSCmsException e) {
			ms_log.error("Exception while trying to create Relationship Proxy");
			throw new RuntimeException(e); // NOPMD by agent on 11/28/06 9:25 AM
		}
		return resultDoc;
	}
   ...
   private static final Log ms_log = LogFactory
         .getLog(PSBuildOwnerRelationshipsFromIdsExit.class);
